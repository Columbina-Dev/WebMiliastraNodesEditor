...
测试版（不定时启用） https://beta.miliastra.columbina.dev/ 。
...
![Screenshot-02](/media/tut2.png)  
  1. 窗口：
    ...
    - 返回主页：返回到[主页介绍](#主页介绍)
  2. 文件：
    - 保存项目：手动保存当前打开的节点图项目到浏览器本地储存
    - 编辑项目信息：修改当前打开的节点图项目名称
    - 导出项目：导出当前打开的节点图项目为Zip文件，可切换设备导入
...
(节点图乱做的，仅供展示style)
  ...
  11. 调整画布缩放
  12. 撤销上一步操作（Ctrl+Z）
  13. 重做上一步操作（Ctrl+Shift+Z）
  14. 保存此次所有编辑过的节点图到浏览器本地储存
  15. 另存为：复制此节点图到当前项目的其他路径
  16. 导出此节点图为 `.server.json` / `.client.json` 文件
  17. (未在图内，介于9和10之间，在客户端节点图编辑器可见)调整执行间隔时间，无任何用处，仅用于还原官方编辑器。
